    ...
    def learn_policy(
        ...
        for i in range(num_episodes):
            ...
            while True:
                ...
                td_error = reward + gamma * self.qtable[next_state, next_action] - self.qtable[state, action]

                self.etable = self.etable * gamma * lambda_value
                self.etable[state, action] = 1

                self.qtable += learning_rate*td_error*self.etable 
        ...
        np.save("qvalues_taxi_sarsa_lambda_grading", np.ar)
