...
def phi(state):
    ...
    for i in range(0, 15):
        dot_product = np.dot(multipliers[i], state)
...
def create_multipliers():
    ...
    coefficients = np.zeros((16, 2))
    for row in range(0, 16):
        x = row // 4
        y = row % 4
        coefficients[row, 0] = x
        coefficients[row, 1] = y
...
def SARSA_Learning():
    ...
    for ep in range(num_episodes):
        ...
        for t in range(num_timesteps):
            ...
            #update your eligibility trace using features
            e*= gamma * Lambda
            for i in range(0, len(features)):
                e[i][action] += features[i]

            # update your td_error, using observed reward and action-value function
            td_error = reward - action_value(state, action)

            #get next action
            next_action = learning_policy(next_state)

            # update your td_error
            td_error += gamma*action_value(next_state, next_action)
